...
- Never change any city name that appears — keep every reference to "${targetCity || '<the target city>'}" exactly as written.
...
${content}`
  ...
  const cleaned = targetCity
    ? enforceCityDisplayInHtml(stripped, parseCity(targetCity))
    : stripped
