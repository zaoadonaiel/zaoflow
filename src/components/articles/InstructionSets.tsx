...
interface InstructionSetsProps {
  ...
  /**
   * Preselect the 800–1000 word set on a new article. Off when editing, so
   * opening an existing article never quietly rewrites its instructions.
   */
  autoSelectDefault?: boolean
}

/**
 * The house default. Matched on the name rather than an id because the set is
 * user-created — anything named like "800-1000", "800 - 1000 words" counts.
 */
function isDefaultSet(set: ArticleInstruction): boolean {
  return /800\s*[-–—to]*\s*1000/i.test(set.name)
}

export default function InstructionSets({ selectedId, onSelect, autoSelectDefault }: InstructionSetsProps) {
  ...
  // Fires once per mount, and only while nothing is selected — so it seeds a
  // fresh article without fighting a choice the user has already made.
  const seeded = useRef(false)
  useEffect(() => {
    if (!autoSelectDefault || seeded.current || selectedId || !sets.length) return
    const preferred = sets.find(isDefaultSet)
    if (preferred) {
      seeded.current = true
      onSelect(preferred)
    }
  }, [autoSelectDefault, sets, selectedId, onSelect])
    ...
    <>
      <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
        ...
        <button
          type="button"
          onClick={openCreate}
          title="New instruction set"
          aria-label="New instruction set"
          className="flex items-center justify-center w-7 h-7 rounded-lg border border-dashed border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400 hover:border-brand-400 hover:text-brand-600 dark:hover:text-brand-400 transition-colors"
        >
