...
interface Props {
  ...
  /** Optional hook after a successful push, so callers can close/refresh. */
  onSent?: (result: { siteId: string; siteName: string; mediaId: number }) => void
}

/**
 * A small dialog for pushing a library image into a WordPress site's media
 * library. One picker, one Send, one call: no re-generation, no re-upload of
 * bytes the library already holds.
 */
export default function SendToWordPressModal({
  open, onClose, imageUrl, fallbackAlt, sites, defaultSiteId, onSent,
}: Props) {
  ...
  async function send() {
    ...
    try {
      ...
      toast.success(`Sent to ${site?.name || 'WordPress'} — media #${data.mediaId}`)
      onSent?.({ siteId, siteName: site?.name || '', mediaId: data.mediaId })
      onClose()
    } catch (err) {
